...
// Case event timelines, generated by the vault build (Cases/_timeline.json).
// Optional ?case=<name> returns a single case; otherwise all cases are returned.
app.get('/api/timeline', (req, res) => {
  const vaultPath = loadConfig().vaultPath
  const timelinePath = path.join(vaultPath, 'Cases', '_timeline.json')

  let payload: { cases: Array<{ name: string }> }
  try {
    payload = JSON.parse(fs.readFileSync(timelinePath, 'utf-8'))
  } catch {
    res.status(404).json({ error: 'No timeline found. Rebuild the vault (npm run build) to generate it.', cases: [] })
    return
  }

  const caseName = (req.query.case as string || '').trim()
  if (caseName) {
    const found = payload.cases.find(c => c.name.toLowerCase() === caseName.toLowerCase())
    if (!found) {
      res.status(404).json({ error: `Case not found: ${caseName}` })
      return
    }
    res.json(found)
    return
