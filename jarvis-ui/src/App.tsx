...
function App() {
  ...
  // Isolate-on-select: when a node is selected, hide everything except it and its neighbours.
  const [isolateOnSelect, setIsolateOnSelect] = useState(() => {
    try { return localStorage.getItem('jarvis-isolate-on-select') !== 'false' } catch { return true }
  })
  ...
  // A node + its directly-linked neighbours.
  const neighboursOf = useCallback((nodeId: string): Set<string> => {
  ...
  // Isolate-on-select: hide unrelated nodes when a node is selected.
  // Skips when an explicit [H] focus lock is active (that takes precedence).
  useEffect(() => {
    if (focusMode) return
    if (!isolateOnSelect || !selectedNode || !graphData) {
      setFocusLockedNodeIds(null)
      return
    }
    setFocusLockedNodeIds(neighboursOf(selectedNode.id))
  }, [selectedNode, isolateOnSelect, graphData, focusMode, neighboursOf])
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#000', position: 'relative', overflow: 'hidden' }}>
      ...
      <Settings
        ...
        isolateOnSelect={isolateOnSelect}
        onIsolateToggle={(v) => {
          setIsolateOnSelect(v)
          try { localStorage.setItem('jarvis-isolate-on-select', String(v)) } catch { /* storage unavailable */ }
        }}
        graphShape={graphShape}
      ...
      {/* Timeline button — opens the case timeline (preselecting a Case node if one is selected) */}
      <button
        onClick={() => setTimelineOpen(true)}
        title="Case event timelines"
        style={{
          position: 'fixed',
          top: 220,
          left: 16,
      ...
      <TimelinePanel
        open={timelineOpen}
        initialCase={
          selectedNode?.folder === 'Cases'
            ? selectedNode.label.replace(/\s+—\s+Case File$/, '')
            : null
        }
